...
def train_deepwalk_embedding(walks, iteration=None):
    if iteration is None:
        iteration = 100
    model = Word2Vec(walks, vector_size=200, window=5, min_count=0, sg=1, workers=4, epochs=iteration)
    return model
...
def train_embedding(current_embedding, walks, layer_id, iter=10, info_size=10, base_weight=1):
    ...
    return new_model.in_base, new_model.in_tran, new_model.in_local, new_model.wv.index_to_key
...
def train_batch(model, sentences, alpha, limitation, base_weight, work=None):
    ...
    for sentence in sentences:
        word_vocabs = [model.key_to_index[w] for w in sentence if w in model.key_to_index and
                        model.key_to_index[w].sample_int > model.random.rand() * 2**32]
        for pos, word in enumerate(word_vocabs):
            ...
            for pos2, word2 in enumerate(word_vocabs[start:(pos + model.window + 1 - reduced_window)], start):
                ...
                if pos2 != pos:
                    train_pair(model, model.wv.index_to_key[word.index], word2.index, alpha, limitation=limitation, base_weight=base_weight)
...
def train_pair(model, word, context_index, alpha, limitation, base_weight, learn_vectors=True, learn_hidden=True,
    ...
    if word not in model.key_to_index:
        return
    predict_word = model.key_to_index[word]  # target word (NN output)
...
class MNE(utils.SaveLoad):
    ...
    def make_cum_table(self, power=0.75, domain=2**31 - 1):
        ...
        for word_index in range(vocab_size):
            train_words_pow += self.key_to_index[self.wv.index_to_key[word_index]].count**power
        cumulative = 0.0
        for word_index in range(vocab_size):
            cumulative += self.key_to_index[self.wv.index_to_key[word_index]].count**power
    ...
    def create_binary_tree(self):
        ...
        """
        logger.info("constructing a huffman tree from %i words", len(self.key_to_index))

        # build the huffman tree
        heap = list(itervalues(self.key_to_index))
        heapq.heapify(heap)
        for i in range(len(self.key_to_index) - 1):
            min1, min2 = heapq.heappop(heap), heapq.heappop(heap)
            heapq.heappush(heap, Vocab(count=min1.count + min2.count, index=i + len(self.key_to_index), left=min1, right=min2))
        ...
        if heap:
            ...
            while stack:
                ...
                if node.index < len(self.key_to_index):
                    # leaf node => store its path from the root
                    node.code, node.point = codes, points
                    max_depth = max(len(codes), max_depth)
                else:
                    # inner node => continue recursion
                    points = array(list(points) + [node.index - len(self.key_to_index)], dtype=uint32)
    ...
    def scale_vocab(self, min_count=None, sample=None, dry_run=False, keep_raw_vocab=False, trim_rule=None, update=False):
        ...
        if not update:
            ...
            for word, v in iteritems(self.raw_vocab):
                if keep_vocab_item(word, v, min_count, trim_rule=trim_rule):
                    ...
                    retain_total += v
                    if not dry_run:
                        self.key_to_index[word] = Vocab(count=v, index=len(self.wv.index_to_key))
            ...
            for word, v in iteritems(self.raw_vocab):
                if keep_vocab_item(word, v, min_count, trim_rule=trim_rule):
                    if word in self.key_to_index:
                        ...
                        pre_exist_total += v
                        if not dry_run:
                            self.key_to_index[word].count += v
                    else:
        ...
        for w in retain_words:
            ...
            if word_probability < 1.0:
                ...
                downsample_total += v
            if not dry_run:
                self.key_to_index[w].sample_int = int(round(word_probability * 2**32))
    ...
    def finalize_vocab(self, update=False):
        ...
        if self.null_word:
            ...
            v.index = len(self.key_to_index)
            self.wv.index_to_key.append(word)
            self.key_to_index[word] = v
    ...
    def sort_vocab(self):
        ...
        self.wv.index_to_key.sort(key=lambda word: self.key_to_index[word].count, reverse=True)
        for i, word in enumerate(self.wv.index_to_key):
            self.key_to_index[word].index = i
    ...
    def reset_from(self, other_model):
        ...
        """
        self.key_to_index = other_model.vocab
        self.wv.index_to_key = other_model.index2word
    ...
    def train(self, sentences, total_words=None, word_count=0,
        ...
        logger.info(
            ...
            self.workers, len(self.key_to_index), self.layer1_size, self.sg,
    ...
    def update_weights(self):
        ...
        gained_vocab = len(self.key_to_index) - len(self.wv.vectors)
        newsyn0 = empty((gained_vocab, self.vector_size), dtype=REAL)

        # randomize the remaining words
        for i in range(len(self.wv.vectors), len(self.key_to_index)):
            # construct deterministic seed from word AND seed argument
            newsyn0[i-len(self.wv.vectors)] = self.seeded_vector(self.wv.index_to_key[i] + str(self.seed))
        self.wv.vectors = vstack([self.wv.vectors, newsyn0])
        ...
        self.syn0_lockf = ones(len(self.key_to_index), dtype=REAL)  # zeros suppress learning

    def reset_weights(self):
        """Reset all projection weights to an initial (untrained) state, but keep the existing vocabulary."""
        logger.info("resetting layer weights")
        self.wv.vectors = empty((len(self.key_to_index), self.vector_size), dtype=REAL)
        self.in_base = empty((len(self.key_to_index), self.vector_size), dtype=REAL)
        self.in_local = empty((len(self.key_to_index), self.small_size), dtype=REAL)
        self.in_tran = zeros((self.small_size, self.vector_size), dtype=REAL)
        # randomize weights vector by vector, rather than materializing a huge random matrix in RAM at once
        if self.initial_embedding is None:
            for i in range(len(self.key_to_index)):
                # construct deterministic seed from word AND seed argument
                self.wv.vectors[i] = self.seeded_vector(self.wv.index_to_key[i] + str(self.seed))
                self.in_base[i] = self.seeded_vector(self.wv.index_to_key[i] + str(self.seed))
                self.in_local[i] = self.seeded_vector2(self.wv.index_to_key[i] + str(self.seed))
        else:
            if self.initial_embedding['tran'] is not None:
                self.in_tran = self.initial_embedding['tran']
                for i in range(len(self.key_to_index)):
                    if self.wv.index_to_key[i] in self.initial_embedding['addition']:
                        self.in_local[i] = self.initial_embedding['addition'][self.wv.index_to_key[i]]
            for i in range(len(self.key_to_index)):
                if self.wv.index_to_key[i] in self.initial_embedding['base']:
                    self.wv.vectors[i] = self.initial_embedding['base'][self.wv.index_to_key[i]]
                    self.in_base[i] = self.initial_embedding['base'][self.wv.index_to_key[i]]
                else:
                    self.wv.vectors[i] = self.seeded_vector(self.wv.index_to_key[i] + str(self.seed))
                    self.in_base[i] = self.seeded_vector(self.wv.index_to_key[i] + str(self.seed))
                self.in_local[i] = self.seeded_vector2(self.wv.index_to_key[i] + str(self.seed))
        if self.hs:
            self.syn1 = zeros((len(self.key_to_index), self.layer1_size), dtype=REAL)
        if self.negative:
            self.syn1neg = zeros((len(self.key_to_index), self.layer1_size), dtype=REAL)
        self.wv.vectorsnorm = None
        self.syn0_lockf = ones(len(self.key_to_index), dtype=REAL)  # zeros suppress learning
        ...
        with utils.smart_open(fname) as fin:
            ...
            if binary:
                ...
                for line_no in range(vocab_size):
                    ...
                    if word in self.key_to_index:
                        overlap_count += 1
                        self.wv.vectors[self.key_to_index[word].index] = weights
                        self.syn0_lockf[self.key_to_index[word].index] = lockf  # lock-factor: 0.0 stops further changes
            else:
    ...
    def estimate_memory(self, vocab_size=None, report=None):
        ...
        vocab_size = vocab_size or len(self.key_to_index)
        report = report or {}
    ...
    def __str__(self):
        return "%s(vocab=%s, size=%s, alpha=%s)" % (self.__class__.__name__, len(self.wv.index_to_key), self.vector_size, self.alpha)
    ...
    def load(cls, *args, **kwargs):
        ...
        for v in model.key_to_index.values():
        ...
        if not hasattr(model, 'syn0_lockf') and hasattr(model, 'syn0'):
            model.syn0_lockf = ones(len(model.wv.vectors), dtype=REAL)
    ...
    def _load_specials(self, *args, **kwargs):
        ...
        if not hasattr(self, 'wv'):
            wv = KeyedVectors()
            wv.vectors = self.__dict__.get('syn0', [])
            wv.vectorsnorm = self.__dict__.get('syn0norm', None)
            key_to_index = self.__dict__.get('vocab', {})
            wv.index_to_key = self.__dict__.get('index2word', [])
            self.wv = wv
